...
* **Técnicas e Práticas Empregadas:**
    * **Prototipação Rápida via Frontend (UI-First) com Base no Figma:** Concepção da base de estilo, tipografia e tokens no Figma, com estabelecimento e validação dos designs de interface e formulários diretamente no Frontend (React SPA / Tailwind CSS) consumindo dados mockados.
...
* **Técnicas e Práticas Empregadas:**
    ...
    * **Sessões Guiadas de Homologação de Interface no Frontend:** Navegação assistida com a cliente Maria Beatryz na interface interativa do Frontend (com apoio da base visual do Figma) para coleta formal de aceite sociotécnico antes da codificação da lógica de regras e persistência.
...
| **Fase 2: Design do Usuário**<br>*(Iteração 2: Prototipagem e Validação)* | Especificação, Validação e Gestão de Requisitos | Prototipagem Rápida Evolutiva no Frontend (UI-First), Sessão JAD de Aceite de Telas, Especificação Guiada. | Base visual de apoio no Figma; designs de tela estabelecidos diretamente no Frontend (React SPA) com dados mockados; Histórias de Usuário BDD; Validação assistida. | Interfaces funcionais navegáveis no Frontend homologadas por Maria Beatryz; Backlog INVEST e DoR atendido para implementação da lógica. |
| **Fase 3: Construção**<br>*(Iteração 3: Core, Lógica e RVT em Campo)* | Especificação Detalhada e Verificação | Construção Rápida de Componentes, Integração Contínua (CI), Testes Automatizados. | Implementação da lógica de negócio e persistência conectando as telas já validadas; Desenvolvimento React/Supabase; Testes unitários; Code Review via PRs. | Módulo funcional de Cadastro e Lançamento de RVT em campo (CAR-01 e CAR-02) com persistência publicado em homologação. |
| **Fase 3: Construção**<br>*(Iteração 4: Motor Financeiro)* | Análise de Exceções e Validação Incremental | Desenvolvimento Incremental, Validação Aritmética, *Continuous Delivery*. | Validação de fórmulas de folha e comissões com dados reais da DUOC; Codificação da lógica de cálculo e APIs financeiras; Testes automatizados. | Motor de Cálculo de Folha e Reembolsos de Despesas (CAR-03 e CAR-04) integrado às interfaces e validado com a cliente. |
...
| `2.1` | 21/09/2026 | Ajuste da estratégia de representação e validação de requisitos: UI-First com base no Figma e designs via Frontend, postergando lógica e persistência para após validação do DoR | Matheus Ribeiro Szervinsk | Equipe Cascata Ágil |